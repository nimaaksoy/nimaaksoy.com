...
import { getSiteAnalytics, hasAnalyticsCredentials } from "@/lib/site-analytics";
...
export default async function StatsPage() {
  ...
  const maxDailyViews = Math.max(1, ...(data?.byDay.map((day) => day.views) || []));
        ...
        <div className="mx-auto max-w-[1180px]">
          <header className="border-b border-[#1F1F1F] pb-8">
            ...
            <p className="mt-5 max-w-2xl font-monroe text-[20px] italic leading-[1.65] text-[#9A9A9A]">
              Aggregate site analytics across home, Radar, tools, prompts, and project pages.
            </p>
          </header>
          ...
          {!configured ? (
            <div className="mt-10 rounded-[8px] border border-[#1F1F1F] bg-[#111111] p-6 md:p-8">
              ...
              <p className="mt-4 max-w-3xl font-jetbrains text-[13px] leading-[1.9] text-[#9A9A9A]">
                Create a PostHog project and add POSTHOG_KEY, POSTHOG_PROJECT_ID, and
                POSTHOG_PERSONAL_KEY in production. Once traffic starts flowing, this page
                will show live aggregate stats.
              </p>
            </div>
          ) : null}
                    ...
                    <div key={day.date} className="flex min-w-0 flex-1 flex-col items-center gap-3">
                      <div className="flex h-52 w-full items-end">
                        <div
                          ...
                          style={{ height: `${Math.max(3, (day.views / maxDailyViews) * 100)}%` }}
          ...
          <p className="mt-8 font-jetbrains text-[11px] leading-[1.9] text-[#7F7F7F]">
            First-party PostHog analytics, queried server-side and published as aggregate
            numbers. Refreshed every two minutes.
          </p>
        </div>
      </section>
    </SiteChrome>
